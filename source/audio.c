...
void audio_music_load() {
  ...
  while (runThread) {
    ...
    // swap data pointers
    music.second_data = temp_data;
    // now the "first" is playing, and the second is not yet added
    // reset the buffer struct
    memset(music.second, 0, sizeof(ndspWaveBuf));
    music.second->data_vaddr = music.second_data;
    music.second->nsamples = BUFFER_SIZE;
    music.second->looping = false;
    music.second->status = NDSP_WBUF_FREE;
...
void audio_fini() {
  ...
  free(music.first);
  free(music.second);
  if (music_loaded) ov_clear(music.file);
  ndspExit();
}
...
void audio_music_check() {
  ...
  // sizes
  if (music.last_check > cur_sample && !ndspChnIsPaused(music.chnl)) {
